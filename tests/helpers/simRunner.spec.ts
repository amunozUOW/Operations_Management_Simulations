...
test('loads Supply Chain without console errors', async ({ page }) => {
  ...
  await expect(page.locator('#btnNextTurn')).toBeVisible();
  expect(errors).toEqual([]);
});

test('clears localStorage between loads', async ({ page }) => {
  await loadSim(page, '07 Supply Chain/Supply Chain.html');
  await page.evaluate(() => localStorage.setItem('ops802_supplychain_v1', 'STALE'));
  await loadSim(page, '07 Supply Chain/Supply Chain.html');
  const v = await page.evaluate(() => localStorage.getItem('ops802_supplychain_v1'));
